function clinical_table(pt,stats)
...
for whichPt = whichPts
    ...
    n_elecs = length(pt(whichPt).new_elecs.electrodes);
    n_strips = 0;
    n_depths = 0;
    n_grids = 0;
    for i = 1:length(pt(whichPt).new_elecs.electrodes)
        if strcmp(pt(whichPt).new_elecs.electrodes(i).type,'S') == 1
            n_strips = n_strips + 1;
        elseif strcmp(pt(whichPt).new_elecs.electrodes(i).type,'G') == 1
            n_grids = n_grids + 1;
        elseif strcmp(pt(whichPt).new_elecs.electrodes(i).type,'D') == 1
            n_depths = n_depths + 1;
        else
            error('What\n');
        end
...
T = table(char(name),char(sex),char(ageOnset),char(ageSurg),char(soz),...
    char(path),char(outcome),char(grids),char(strips),char(depths))
